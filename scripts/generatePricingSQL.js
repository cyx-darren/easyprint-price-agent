/**
 * Generate pricing SQL INSERT statements in batches
 */
...
// Product ID mapping from Supabase
const productIds = {
...
const values = batch.map(r => {
  ...
  if (!productId) {
    console.error('-- Missing product ID for:', r.product_name);
    return null;
  }
...
if (values.length > 0) {
  console.log(`INSERT INTO pricing (product_id, product_name, print_option, lead_time_type, lead_time_days_min, lead_time_days_max, quantity, unit_price, is_moq) VALUES`);
  console.log(values.join(',\n') + ';');
}

console.error(`-- Batch ${batchNum}: records ${start}-${end} of ${data.records.length}`);
