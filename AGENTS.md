...
- **Corporate gift catalogue prices, tiers, MOQ** -> `pricing` joined to `products`. Website lookups resolve `products.website_product_id -> products.id -> pricing.product_id`, then filter by specs (`print_option`, `lead_time_type`, `quantity`/`is_moq`) — see "How a website_product_id resolves to a price" in `README.md`.
- **Custom heat transfer (dye sublimation) lanyards** (1.5cm/2cm/2.5cm x 90cm) -> the live view `heat_transfer_lanyard_prices` (filter `width_mm` 15/20/25), or `calculate_heat_transfer_lanyard_price(attachment, qty, freight, width_mm)` for off-grid quantities. The view exposes `product_id` referencing the lanyard's `products` row (website_product_id `ece775be-d6de-409f-a129-152d850dba26`), so the same website_product_id chain works here; the lanyard has no `pricing` rows. These prices are **computed on read** from `lanyard_component_costs`, `lanyard_freight_costs`, `lanyard_profit_margins`, and `global_overseas_pricing` — never import computed lanyard selling prices as static rows; update the raw-input tables instead and prices follow automatically.
...
- **Supplier print/decoration charges** -> `mygift_charges` / `sunprint_charges` / `plsilkscreen_charges` (+ `*_category_mappings`), `venue31_charges`. Ultifresh items resolve via `ultifresh_products.categories` -> `sunprint_category_mappings` rows with `supplier_name = 'ULTIFRESH'`.
